...
ModPicker helps someone choose automotive parts for a specific car or an engine/platform family, understand the evidence behind each result, compare current seller observations, and turn a shortlist into a practical build sheet.

This document is the durable product brief. The implementation review and dated changes live in [REVIEW_AND_PLAN.md](REVIEW_AND_PLAN.md). The operational runbook lives in [OPERATIONS.md](OPERATIONS.md), and the complete catalog contribution procedure lives in [MANUAL_CATALOG.md](MANUAL_CATALOG.md).
...
3. **Useful uncertainty.** Exact source-listed fitment, platform-related results, unknown prices, stale observations, and pending evidence have visibly different states.
...
5. **Deterministic growth.** Scheduled updates, manual JSON contributions, and future source adapters use documented validation and provenance rules. No ChatGPT or other language model is required to add catalog records or calculate scores.
6. **Reviewable operations.** Every automated refresh produces a status record, a review queue, canonical JSON, a browser fallback bundle, and a diff that can be inspected before release.
...
1. Expand exact vehicle applications only when a primary or clearly identified source supports them.
2. Add more permitted structured product feeds and exact fitment mappings before adding more platform families.
3. Add a review-queue view for maintainers so ambiguous candidates can be resolved without editing generated files.
4. Add regression checks for every new platform: valid aliases, at least one application, category scope, and a browser selection path.
5. Consider account-backed build synchronization only after the local export/import flow remains stable and privacy expectations are documented.

Each completed priority should update this document, `REVIEW_AND_PLAN.md`, the relevant tests, and the operational runbook in the same change.
