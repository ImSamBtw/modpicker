...
window.MP_APPLY(parts.filter(p=>p.status==='active').map(p=>({id:p.id,brand:p.manufacturer,name:p.name,category:p.category,description:p.description,official_url:p.official_url,manufacturer_part_number:p.manufacturer_part_number,install:p.metadata?.install,fitments:snapshot[p.id]?.fitments??fitments.filter(f=>f.part_id===p.id),sources:sources.filter(s=>s.part_id===p.id),offers:offers.filter(o=>o.part_id===p.id).map(o=>({...o,vendor:vendorMap[o.vendor_id]})),ranking:scores.find(s=>s.part_id===p.id&&s.methodology_version==='published_reviews_v2')||snapshot[p.id]?.ranking})),vehicles,window.MODPICKER_PLATFORMS||[]);
