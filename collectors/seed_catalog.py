    ...
    def run(self, parts):
        ...
        for p in parts:
            ...
            if url:
                ...
                sources.append(SourceRecord.make(
                    ...
                    metadata={
                        ...
                        'catalog_seed':True,
                        'fitment_status':p.get('fitment_status','unknown'),
            ...
            if ph.get('url') and ph.get('vendor'):
                try: price=float(ph['price']) if ph.get('price') is not None else None
                except (TypeError,ValueError): price=None; warnings.append(f"{p['id']}: invalid price_hint")
